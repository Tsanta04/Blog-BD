...
{
  private isPlainObjectWithNumericKeys(obj: any): boolean {
    ...
    return next.handle().pipe(
      map((data: any) => {
        ...
        if (isPaginated && data?.data && data?.pagination) {
          ...
          if (this.isPlainObjectWithNumericKeys(rest)) {
            // Convert indexé object to array
            formattedData = Object.values(rest);
          } else {
            formattedData = rest;
          }
        }
